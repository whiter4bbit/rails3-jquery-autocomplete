...
(function(jQuery)
{
  ...
  jQuery.railsAutocomplete.currentResults = [];
      ...
      $(e).autocomplete({
        source: function( request, response ) {
          $.getJSON( $(e).attr('data-autocomplete'), {
            ...
            jQuery.railsAutocomplete.currentResults = arguments[0];
            $(e).trigger('railsAutocomplete.dataLoaded');
            var found = false;
            var enteredValue = $(e).val();
            for(var i = 0; i < arguments[0].length; i++) {
               if (arguments[0][i].value == enteredValue) {
                 found = true;
               }
            }
            if (!found) {
